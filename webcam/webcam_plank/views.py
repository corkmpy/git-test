...
timer_started = False  # 타이머 시작 플래그
start_time = 0  # 타이머 시작 시간
elapsed_time = 0  # 경과 시간

# 이미지 불러오기
image_path = os.path.join(settings.BASE_DIR, 'static/images/PlankPose.jpg')
...
def classifyPose(landmarks, output_image, display=False):
    global prev_label, max_similarity, timer_started, start_time, elapsed_time
    ...
    # 'Accurate pose'인 경우
    if similarities['left_knee'] > 75 and similarities['right_knee'] > 75:
        if similarities['waist'] > 50:
            if similarities['left_elbow'] > 40 and similarities['right_elbow'] > 40:  # 임계값 조정 가능
                label = 'Accurate pose'
                color = (0, 255, 0)
    prev_label = label
...
def gen(camera):
    ...
    while True:
        ...
        cv2.putText(frame, f'Time: {elapsed_time:.2f}s', (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
similarities = []
def cleanup(user):
    global average_similarity, similarities

    if similarities:
        average_similarity = sum(similarities) / len(similarities)

    # 모델에 데이터 저장
    Plank_data.objects.create(
        user = user,
        average_similarity=average_similarity,
    )        
def plank_view(request):
    return render(request, 'plank/plank.html')
...
def stop_stream(request):
    if request.method == 'POST':
        ...
        latest_data = Plank_data.objects.filter(user=user).order_by('-timestamp').first()
        return JsonResponse({'status': 'success', 'average_similarity': latest_data.average_similarity})
