...
max_similarity = {'left_elbow': 0, 'right_elbow': 0, 'left_knee': 0, 'right_knee': 0}  # 가장 높은 유사도 초기값

# 이미지 불러오기
image_path = os.path.join(settings.BASE_DIR, 'static/images/test6.png')
...
def calculateAngleSimilarity(angle1, angle2):
    ...
    # 코사인 유사도를 계산합니다.
    similarity = (cos_angle1 * cos_angle2 + 1) / 2  # [0, 1] 범위로 정규화
...
def classifyPose(landmarks, output_image, display=False):
    ...
    if similarities['left_knee'] > 75 and similarities['right_knee'] > 75:
        if similarities['left_elbow'] > 50 and similarities['right_elbow'] > 50:  # 임계값 조정 가능
            label = 'Accurate pose'
            color = (0, 255, 0)
...
def gen(camera):
    ...
    while True:
        ...
        cv2.putText(frame, f'Count: {count}', (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
def pushup_view(request):
    return render(request, 'pushup/pushup.html')
...
def stop_stream(request):
    if request.method == 'POST':
        ...
        latest_data = Pushup_data.objects.filter(user=user).order_by('-timestamp').first()
